...
with tab_comp:
    ...
    with sub_shocks:
        ...
        **1,021% de variación**. ¿Cuál modelo mantiene su precisión cuando más importa?
        ...
        from sklearn.metrics import mean_absolute_error as _mae_fn  # inline import

        # ── Rutas CSV nuevos modelos ──────────────────────────────────────────
        _P_SARIMA_CSV  = BASE_DIR / "resultados/gc1/gc1_sarima_predicciones.csv"
        _P_PROPHET_CSV = BASE_DIR / "resultados/gc1/gc1_prophet_predicciones.csv"
        _P_GC2_CSV     = BASE_DIR / "resultados/gc2/gc2_predicciones.csv"

        _SHOCK_DATES = pd.to_datetime([
        ...
        # ── Loader con cache ──────────────────────────────────────────────────
        @st.cache_data(show_spinner=False)
        def _load_pred_csv(path_str: str, date_col: str, pred_col: str):
            p = pathlib.Path(path_str)
            if not p.exists():
                return None
            df = pd.read_csv(p, parse_dates=[date_col])
            df = df.rename(columns={date_col: "fecha", pred_col: "pred"})
            return df[["fecha", "real", "pred"]].sort_values("fecha").reset_index(drop=True)

        # ── Catálogo completo: nombre → (df, color, dash, symbol) ────────────
        _CAT = {
            "GE (principal)":   (_load_pred_csv(str(P_GE_PREDS),       "fecha",        "prediccion"),  "#2e7d32", "solid",       "triangle-up"),
            "XGBoost":          (_load_pred_csv(str(P_XGB_PREDS_CSV),   "fecha",        "pred_xgb"),    "#ff7043", "dash",        "square"),
            "GM v2":            (_load_pred_csv(str(P_GMV2_PREDS_CSV),  "fecha",        "pred_gm_v2"),  "#26a69a", "dot",         "diamond"),
            "GC1-SARIMA":       (_load_pred_csv(str(_P_SARIMA_CSV),     "fecha_evento", "prediccion"),  "#1976d2", "dash",        "circle"),
            "GC1-Prophet":      (_load_pred_csv(str(_P_PROPHET_CSV),    "fecha",        "prediccion"),  "#7b1fa2", "dot",         "star"),
            "GC2-SARIMAX+LSTM": (_load_pred_csv(str(_P_GC2_CSV),       "fecha",        "hibrido"),     "#e64a19", "longdashdot", "pentagon"),
            "TCN":              (_load_pred_csv(str(P_TCN_PREDS_CSV),   "fecha",        "pred_tcn"),    "#f57f17", "dashdot",     "x"),
        }

        # ── Calcular MAE global y en shocks dinámicamente ─────────────────────
        _shock_rows = []
        for _nom, (_df, _col, _dsh, _sym) in _CAT.items():
            if _df is None:
                continue
            _df2 = _df.dropna(subset=["real", "pred"])
            if len(_df2) < 2:
                continue
            _mae_g = _mae_fn(_df2["real"], _df2["pred"])
            _mask  = _df2["fecha"].isin(_SHOCK_DATES)
            _df_sh = _df2[_mask]
            if len(_df_sh) >= 2:
                _mae_s = _mae_fn(_df_sh["real"], _df_sh["pred"])
                _det   = (_mae_s - _mae_g) / _mae_g * 100
            else:
                _mae_s = float("nan")
                _det   = float("nan")
            _shock_rows.append({
        ...
        _df_dyn = pd.DataFrame(_shock_rows)

        # ── Barras agrupadas global vs shock (solo modelos con shocks válidos) ─
        col_s1, col_s2 = st.columns(2)
        with col_s1:
            _v = _df_dyn[_df_dyn["MAE shocks"].apply(lambda x: not (isinstance(x, float) and np.isnan(x)))]
            fig_gs = go.Figure()
            fig_gs.add_trace(go.Bar(
                name="MAE global",
                x=_v["Modelo"],
                y=_v["MAE global"],
                marker_color=_v["_color"].tolist(),
                opacity=0.5,
                text=[f"{v:.4f}" for v in _v["MAE global"]],
                textposition="outside",
            ))
            fig_gs.add_trace(go.Bar(
                name="MAE en shocks",
                ...
                marker_line_color="black",
                marker_line_width=1.2,
                text=[f"{v:.4f}" for v in _v["MAE shocks"]],
                textposition="outside",
            ))
            fig_gs.update_layout(
                barmode="group",
                title="MAE Global vs MAE en meses de shock — todos los modelos",
                yaxis_title="MAE",
                height=430,
                margin=dict(t=55, b=65, l=50, r=20),
                plot_bgcolor="white",
                paper_bgcolor="white",
        ...
        with col_s2:
            _vd = _df_dyn[
                _df_dyn["Deterioro %"].apply(lambda x: not (isinstance(x, float) and np.isnan(x)))
            ].sort_values("Deterioro %")
            _det_colors = [
                "#2e7d32" if v <= 5 else
                "#ff7043" if v <= 20 else
                "#c62828" if v > 0 else
                "#26a69a"
                for v in _vd["Deterioro %"]
            ]
            fig_det = go.Figure(go.Bar(
                x=_vd["Deterioro %"],
                y=_vd["Modelo"],
                orientation="h",
                marker_color=_det_colors,
                text=[f"{v:+.1f}%" for v in _vd["Deterioro %"]],
                textposition="outside",
            ))
            fig_det.add_vline(x=0,  line_color="black",   line_width=1.5)
            fig_det.add_vline(x=5,  line_dash="dash",     line_color="#2e7d32",
                              annotation_text="Verde ≤5%",  annotation_position="top right")
            fig_det.add_vline(x=20, line_dash="dash",     line_color="#ff7043",
                              annotation_text="Rojo >20%",  annotation_position="top right")
            fig_det.update_layout(
                title="Deterioro MAE en shocks<br>(verde ≤5% / naranja ≤20% / rojo >20%)",
                xaxis_title="Δ MAE (%)",
                height=430,
                margin=dict(t=60, b=40, l=150, r=110),
                plot_bgcolor="white",
                paper_bgcolor="white",
                showlegend=False,
            )
        ...
        # ── Gráfico de líneas interactivo — todos los modelos ─────────────────
        st.markdown("#### Visualización interactiva — predicciones en período de test completo")
        st.markdown("*Real en negro grueso · áreas rojas = meses de shock · todos los modelos disponibles*")

        # Serie "Real" unificada con el rango más amplio posible
        _all_reals = []
        for _nom, (_df, *_) in _CAT.items():
            if _df is not None and len(_df) > 0:
                _all_reals.append(_df[["fecha", "real"]])
        _real_unified = (
            pd.concat(_all_reals)
            .drop_duplicates("fecha")
            .sort_values("fecha")
            .reset_index(drop=True)
        ) if _all_reals else None

        if _real_unified is not None:
            fig_comp = go.Figure()

            # Real en negro grueso
            fig_comp.add_trace(go.Scatter(
                x=_real_unified["fecha"],
                y=_real_unified["real"],
                name="Real",
                mode="lines+markers",
            ...
            # Predicciones de cada modelo disponible
            for _nom, (_df, _col, _dsh, _sym) in _CAT.items():
                if _df is None or len(_df) == 0:
                    continue
                _df2 = _df.dropna(subset=["pred"])
                if len(_df2) == 0:
                    continue
                fig_comp.add_trace(go.Scatter(
                    x=_df2["fecha"],
                    ...
                    name=_nom,
                    mode="lines+markers",
                    line=dict(color=_col, width=2, dash=_dsh),
                    marker=dict(size=6, symbol=_sym),
                ))

            # Áreas rojas en meses de shock
            for _sf in _SHOCK_DATES:
                try:
                    fig_comp.add_vrect(
                        x0=_sf, x1=_sf + pd.DateOffset(months=1),
            ...
            _y_max = float(_real_unified["real"].max()) * 1.15
            ...
            fig_comp.update_layout(
                title="Predicciones vs Real — todos los modelos disponibles (áreas rojas = shocks)",
                xaxis_title="Fecha",
                ...
                plot_bgcolor="white",
                paper_bgcolor="white",
                legend=dict(orientation="h", yanchor="bottom", y=-0.52),
                hovermode="x unified",
            )
            ...
            st.warning("No se encontraron archivos de predicciones disponibles.")

        # ── Tabla resumen de robustez — todos los modelos ─────────────────────
        st.markdown("#### Tabla resumen — robustez en shocks")
        _rob_rows = []
        for _, _r in _df_dyn.iterrows():
            _mae_sh = _r["MAE shocks"]
            _det    = _r["Deterioro %"]
            _nan_sh = isinstance(_mae_sh, float) and np.isnan(_mae_sh)
            _nan_dt = isinstance(_det,    float) and np.isnan(_det)
            _mae_sh_str = f"{_mae_sh:.4f}" if not _nan_sh else "N/D"
            _det_str    = f"{_det:+.1f}%"  if not _nan_dt else "N/D"
            if _nan_dt:
                _verd = "N/D (< 2 meses de shock)"
            elif _det <= 5:
                _verd = "✅ Robusto"
            elif _det <= 20:
                _verd = "⚠️ Vulnerable"
            ...
            _rob_rows.append({
                "Modelo":     _r["Modelo"],
                "MAE global": f"{_r['MAE global']:.4f}",
                "MAE shocks": _mae_sh_str,
                "Deterioro":  _det_str,
                "Veredicto":  _verd,
            })
        ...
        st.markdown("""
        <p class='disclaimer'>
        Deterioro calculado dinámicamente desde los CSV: (MAE_shocks − MAE_global) / MAE_global × 100.
        Shocks: 2024-09, 2024-10, 2024-12, 2025-01, 2025-02, 2025-04, 2025-05, 2025-06.
        Modelos con &lt;2 meses de shock en su período de test muestran "N/D".
        GE mantiene +2.3% gracias a variables climáticas NASA POWER y registros INDECI.
        * TCN mejora en shocks pero MAE global = 0.1860 (peor entre modelos complejos).
        </p>
        """, unsafe_allow_html=True)
# TAB 6 — ATTENTION XAI
# ══════════════════════════════════════════════════════════════════════════════
